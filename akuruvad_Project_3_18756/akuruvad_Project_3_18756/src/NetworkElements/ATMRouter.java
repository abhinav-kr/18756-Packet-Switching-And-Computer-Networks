...
public class ATMRouter implements IATMCellConsumer{
	...
	public void receiveCell(ATMCell cell, ATMNIC nic){
		...
		if(cell.getIsOAM()){
			...
			if(pair!=null ) {
				...
				if( this.VCtoVC.containsKey(cell.getVC()) && this.VCtoVC.get(cell.getVC()) == null && this.trace ){
	...
	private void receivedSetup(ATMCell cell, ATMNIC nic){
		...
		if(this.currentConnAttemptNIC != null) {
			this.forwardWAITCell(nic, "WAIT " + destination, cell);
		} else {
			ATMNIC nextNICHop = nextHop.get(destination);
			this.forwardCALLPROCCell(nic, "CALLPROC" + destination, cell);
			if( this.address ==  destination) {
				...
				this.forwardCONNECTCell(nic, "CONNECT "+newVC.toString(), cell);
				this.VCtoVC.put(newVC, null);
			} else {
				currentConnAttemptNIC = nic;
				this.forwardSETUPCell(nextNICHop, "SETUP "+destination.toString(), cell);
			}
		}
	...
	private void receivedConnect(ATMCell cell, ATMNIC nic) {
		...
		this.forwardCONNACKCell(nic, "CONNACK", cell);
		this.forwardCONNECTCell(currentConnAttemptNIC, "CONNECT "+nextAvailableVCNumber, cell);
	...
	private void recieveEnd(ATMCell cell, ATMNIC nic){
		...
		this.forwardENDACKCell(nic, "ENDACK", cell);
		
		if ( NICVCPairWithDestVC == null ) {
			return;
		}
		Integer destVC = NICVCPairWithDestVC.getVC();
		this.forwardENDCONNCell(NICVCPairWithDestVC.getNIC(), "ENDCONN " +destVC.toString(), cell);
	...
	private void receivedWait(ATMCell cell, ATMNIC nic){
		...
		Integer destination = this.getIntFromEndOfString(cell.getData());
		this.forwardSETUPCell(nic, "SETUP "+destination.toString(), cell);
		
		if(this.displayCommands)
		System.out.println("REC WAIT: Router " +this.address+ " received a wait message " + cell.getTraceID());
	}
	
	private void forwardSETUPCell(ATMNIC nic, String signalData, ATMCell cell ) {
	...
	private void forwardCALLPROCCell(ATMNIC nic, String signalData, ATMCell cell ) {
	...
	private void forwardWAITCell(ATMNIC nic, String signalData, ATMCell cell ) {
		ATMCell newCell = new ATMCell(0, signalData, this.traceID++);
		newCell.setIsOAM(true);
	...
	private void forwardCONNECTCell(ATMNIC nic, String signalData, ATMCell cell ) {
		ATMCell newCell = new ATMCell(0, signalData, this.traceID++);
		newCell.setIsOAM(true);
	...
	private void forwardCONNACKCell(ATMNIC nic, String signalData, ATMCell cell ) {
		ATMCell newCell = new ATMCell(0, signalData, this.traceID++);
		newCell.setIsOAM(true);
	...
	private void forwardENDCONNCell(ATMNIC nic, String signalData, ATMCell cell ) {
		ATMCell newCell = new ATMCell(0, signalData, cell.getTraceID());
		newCell.setIsOAM(true);
	...
	private void forwardENDACKCell(ATMNIC nic, String signalData, ATMCell cell ) {
		ATMCell newCell = new ATMCell(0, signalData, cell.getTraceID());
		newCell.setIsOAM(true);
