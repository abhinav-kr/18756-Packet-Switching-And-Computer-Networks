...
public class OtoOLink {
	...
	public void sendPacket(Packet packet, NIC nic){
		if(this.r1NIC.equals(nic)){
			if(this.trace)
				System.out.println("(OtoOLink) Trace: sending packet from router "+r1NIC.toString()+" to "+r2NIC.toString());

			this.r2NIC.receivePacket(packet);
		}
		else if(this.r2NIC.equals(nic)){
			if(this.trace)
				System.out.println("(OtoOLink) Trace: sending packet from router "+r2NIC.toString()+" to "+r1NIC.toString());
