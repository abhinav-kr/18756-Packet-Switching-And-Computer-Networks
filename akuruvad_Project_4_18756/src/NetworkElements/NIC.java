...
public class NIC {
	...
	public void sendPacket(Packet packet, PacketConsumer parent){
		if(this.trace){
			System.out.println("Trace (NIC): Received packet at "+this.parent.toString());
			if(this.link==null)
	...
	public void receivePacket(Packet packet){
		if(this.trace) {
			System.out.println("Packet Received at "+this.parent.toString());
		}
		...
		if (this.buffer.size() == this.bufferSize) {
			if(this.trace) {
				System.out.println("Packet DROPPED at "+this.parent.toString());
			}
		} else {
			this.buffer.add(packet);
		}
	...
	public void sendFromBuffer(){
		for(int i=0; i<this.buffer.size(); i++){
			if(this.trace){
				System.out.println("Sending from NIC's Buffer");
			}
			this.link.sendPacket(this.buffer.get(i), this);
		}
		this.buffer.clear();
		if(this.trace) {
			System.out.println("All packets from buffer sent");
		}
	}
	...
	public Packet getPacketAtIndex(int index) {
		if (this.buffer.size()>index) {
			Packet packet = this.buffer.get(index);
			return packet;
		}
		return null;
	}
	
	public void packetSwitchedSuccessfully() {
		this.buffer.remove(0);
	}
	
	public void packetSwitchedSuccessfullyAtIndex(int i) {
