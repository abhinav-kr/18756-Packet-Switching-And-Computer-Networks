...
public class Computer implements IPConsumer {
	 ...
	 * 
	 * @param nic
	 *            the nic to be added (only one nic per computer)
	 */
	...
	public void sendPacket(String dest, int size) {
		if (this.nic == null)
			System.err
					.println("The computer you are sending from does not have a NIC!");

		if (trace)
			System.out
					.println("(Computer) Trace: sending packet from computer with the IP : "
							+ this.nic.getIPAddress().getHostAddress());
